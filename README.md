...
- 设计图转实景：上传 1 张婚礼设计图/效果图，生成 4 张 `1536x864` 真实现场候选图，不合成上下对比图
...
```bash
IMAGE_PROVIDER=xiaoji
XIAOJI_IMAGE_ENDPOINT=https://xiaoji.baziapi.site/v1/images/generations
XIAOJI_EDIT_ENDPOINT=https://xiaoji.baziapi.site/v1/images/edits
XIAOJI_IMAGE_INPUT_MODE=edit
XIAOJI_EDIT_IMAGE_FIELD=image
XIAOJI_API_KEY=你的_key
XIAOJI_IMAGE_MODEL=gpt-image-2
...
```powershell
$env:IMAGE_PROVIDER="xiaoji"
$env:XIAOJI_IMAGE_ENDPOINT="https://xiaoji.baziapi.site/v1/images/generations"
$env:XIAOJI_EDIT_ENDPOINT="https://xiaoji.baziapi.site/v1/images/edits"
$env:XIAOJI_IMAGE_INPUT_MODE="edit"
$env:XIAOJI_EDIT_IMAGE_FIELD="image"
$env:XIAOJI_API_KEY="你的_key"
...
当前账号可用的生图模型是 `gpt-image-2`。如果服务商后台调整模型名，把 `XIAOJI_IMAGE_MODEL` 改成文档指定的模型。
...
```bash
XIAOJI_IMAGE_INPUT_MODE=json
XIAOJI_REFERENCE_FIELD=reference_images
```
...
如果不用小鸡接口，而是使用 n1n.ai 这类 OpenAI-compatible 接口，把 `.env` 改成下面这样：
...
爆款标题文案和视频提示词整理都使用同一个 `OPENAI_BASE_URL` 下的 `/chat/completions`，但模型分开配置：`COPY_MODEL` 用于小红书标题文案，`MOTION_DIRECTOR_MODEL` 用于视频提示词整理。如果 n1n 后台的文本/视觉模型名不同，只需要分别换成可用模型；如果暂时不想走文案接口，把 `ENABLE_COPY_API=false` 即可。

4K 视频依赖服务商账号和模型实际支持 4K；`PUBLIC_BASE_URL` 必须是服务商能访问到的公网域名，否则后端会进入演示视频占位模式。
...
DESIGN_RENDER_POINT_COST=20
ADMIN_TOKEN=换成管理员私密口令
RECHARGE_PLANS=9.9元=100灵感值;99元=1200灵感值;199元=2600灵感值;399元=5600灵感值
```
...
这会创建账号 `13800138000`，登录码 `246810`，初始 3 点。客户用这个账号和登录码进入网站。

手动充值：

```bash
node scripts/account-admin.mjs recharge 13800138000 90 99元套餐
```
