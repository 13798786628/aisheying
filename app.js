...
const RESOURCE_CATEGORIES = [
  ...
  { key: 'videos', label: '视频', empty: '还没有视频素材。生成单图运镜视频后会出现在这里。' },
  { key: 'comparisons', label: '对比图', empty: '还没有对比图。设计图对比、布置前后对比会归到这里。' },
];
...
const MODE_CONFIG = {
  ...
  design_comparison: {
    label: '设计现场对比图',
    title: '生成设计图与现场图 3:4 对比首图',
    tags: ['#婚礼设计图', '#婚礼现场图', '#婚礼策划', '#婚礼布置', '#备婚参考'],
  },
  design_render_scene: {
    label: '设计图转实景',
...
const DEFAULT_MOTION_STYLE = 'fast_push_in';
const FALLBACK_MOTION_STYLES = [
  { key: 'fast_push_in', label: '快速前推', description: '镜头更有冲击，从全景迅速推向主体' },
  { key: 'gentle_push_in', label: '温柔前推', description: '慢速靠近，氛围柔和稳定' },
  { key: 'pull_back', label: '后拉', description: '从局部慢慢后退，展开空间层次' },
  { key: 'micro_orbit', label: '微环绕', description: '轻微侧向环绕，增加空间感' },
];
...
const els = {
  ...
  motionStyleSection: $('#motionStyleSection'),
  motionStyleGrid: $('#motionStyleGrid'),
  motionPointHint: $('#motionPointHint'),
  videoMotionStyleGrid: $('#videoMotionStyleGrid'),
  // video 页面（独立工作流）
  ...
  videoResultPoster: $('#videoResultPoster'),
...
let motionConfig = { pointCost: 60, durationSeconds: 8, resolution: '4K', referenceLimit: 1, publicBaseConfigured: false, mockMode: false, styles: FALLBACK_MOTION_STYLES };
...
function applyPointCosts(payload = {}) {
  ...
  if (payload.motion && typeof payload.motion === 'object') {
    motionConfig = {
      ...
      referenceLimit: Math.max(1, Number(payload.motion.referenceLimit) || motionConfig.referenceLimit || 1),
  ...
  if (typeof renderVideoStyleButtons === 'function') renderVideoStyleButtons();
  if (typeof videoUpdateGenerateState === 'function') videoUpdateGenerateState();
}

function videoReferenceLimit() {
  return Math.max(1, Number(motionConfig.referenceLimit) || 1);
}
...
function setGenerating(isGenerating) {
  ...
  const idleText = {
    ...
    design_comparison: '生成设计对比图',
    design_render_scene: '生成实景候选图',
    venue_fusion: '生成空地融合图',
    copy_title: '生成标题文案',
    motion_video: `一键生成运镜视频（${motionConfig.pointCost || 60} 灵感值）`,
  }[selectedMode] || '开始生成';
...
function cueVideoVisibleFrame(video, seconds = 0.2) {
  if (!video) return;
  const seek = () => {
    const duration = Number(video.duration);
    if (!Number.isFinite(duration) || duration <= 0) return;
    if (video.currentTime > 0.05) return;
    const targetTime = Math.min(seconds, Math.max(0, duration - 0.05));
...
function setVideoPosterFallback(video, posterEl, posterUrl = '') {
  if (!posterEl) return;
  ...
  if (posterSrc) {
    posterEl.onload = () => {
      if (posterEl.src) posterEl.classList.remove('hidden');
    };
    posterEl.onerror = () => {
      posterEl.classList.add('hidden');
    ...
    if (video) video.poster = posterSrc;
  } else {
    posterEl.removeAttribute('src');
    posterEl.classList.add('hidden');
    if (video) video.removeAttribute('poster');
...
function bindVideoPosterFallback(video, posterEl, posterUrl = '') {
  if (!video) return;
  setVideoPosterFallback(video, posterEl, posterUrl);
  const hidePoster = () => {
    if (video.readyState >= 2) posterEl?.classList.add('hidden');
  };
  cueVideoVisibleFrame(video);
...
function ensureMotionStyleModal() {
  ...
  overlay.innerHTML = `
    <div class="motion-modal-card" role="dialog" aria-modal="true" aria-labelledby="motionModalTitle">
      ...
      <div class="motion-modal-header">
        ...
        <h3 id="motionModalTitle" class="motion-modal-title">一键生成单图运镜视频</h3>
        <p class="motion-modal-meta">用当前图片作为唯一参考图，选择一种镜头运动生成 8 秒电影感空景视频。优先选择无人、清晰、无水印的婚礼现场图。</p>
      </div>
      <div class="motion-modal-source">
        <img alt="" />
        <div><strong></strong><span></span></div>
      </div>
      <div class="motion-modal-grid"></div>
      <div class="motion-modal-refs hidden">
        <div class="motion-modal-ref-head">
          <strong>单图运镜</strong>
          <span>当前模式只使用 1 张参考图</span>
        </div>
        <div class="motion-modal-ref-grid"></div>
      </div>
      <div class="motion-modal-footer">
        <span class="motion-modal-status"></span>
        <button class="motion-modal-submit" type="button">一键生成运镜视频（60 灵感值）</button>
      </div>
    </div>
  `;
  document.body.appendChild(overlay);
  motionStyleModal = {
    ...
    styleGrid: overlay.querySelector('.motion-modal-grid'),
...
function openMotionStyleModal(item) {
  ...
  modal.submitBtn.textContent = `一键生成运镜视频（${motionConfig.pointCost || 60} 灵感值）`;
  ...
  if (modal.styleGrid) {
    ...
    modal.styleGrid.innerHTML = '';
    styles.forEach((style) => {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = `motion-style-btn ${style.key === modal.style ? 'active' : ''}`;
      btn.dataset.styleKey = style.key;
      btn.innerHTML = `<strong>${style.label}</strong><span>${style.description || ''}</span>`;
      btn.addEventListener('click', () => {
        modal.style = style.key;
        modal.styleGrid.querySelectorAll('.motion-style-btn').forEach((el) => {
          el.classList.toggle('active', el.dataset.styleKey === modal.style);
        });
      });
      modal.styleGrid.appendChild(btn);
    });
  }
...
async function submitMotionFromResource() {
  ...
  try {
    const response = await fetch(apiUrl(`/api/resources/${encodeURIComponent(resourceId)}/motion-video`), {
      ...
      body: JSON.stringify({
        filename,
        motion_style: motionStyleModal.style || DEFAULT_MOTION_STYLE,
    ...
    videoState.style = motionStyleModal.style || DEFAULT_MOTION_STYLE;
    const currentStyle = (motionConfig.styles || FALLBACK_MOTION_STYLES).find((style) => style.key === videoState.style);
    ...
    videoRenderLogs([
      ...
      `[input] 单图生视频：使用当前资源图`,
      `[motion] 运镜：${currentStyle?.label || videoState.style || '快速前推'}`,
      `[queue] 任务 id=${data.id}`,
    ]);
    ...
    motionStyleModal.submitBtn.textContent = `一键生成运镜视频（${motionConfig.pointCost || 60} 灵感值）`;
  }
}
...
function createResourceMotionButton(item) {
  ...
  button.setAttribute('aria-label', `用这张图一键生成运镜视频`);
...
function aspectRatioForItem(item, mode = selectedMode) {
  ...
  if (mode === 'cinematic_storyboard' || mode === 'setup_comparison' || mode === 'design_comparison' || mode === 'design_render_scene') return '16 / 9';
  return '1 / 1';
}
...
function prepareResultPlaceholders() {
  ...
  if (els.overviewKicker && els.overviewTitle) {
    ...
    const designComparison = selectedMode === 'design_comparison';
    const designRender = selectedMode === 'design_render_scene';
    const venueFusion = selectedMode === 'venue_fusion';
    els.overviewKicker.textContent = copyOnly ? '文案结果' : (setupComparison || designComparison ? '3:4 对比图' : (designRender ? '真实现场候选图' : (venueFusion ? '融合结果' : '合成预览')));
    els.overviewTitle.textContent = copyOnly
      ? '爆款标题文案'
      : (setupComparison ? '布置前后对比图' : (designComparison ? '设计现场对比图' : (designRender ? '4 张现场候选图' : (venueFusion ? '空地婚礼融合图' : '分镜总览 / 爆款首图'))));
  }
...
function mockTileSvg(index, mode) {
  const palettes = {
    ...
    design_comparison: ['#1d1d20', '#d8c3ff', '#f0c2b5', '#a7f3d0'],
  ...
  let width = (mode === 'cinematic_storyboard' || mode === 'setup_comparison' || mode === 'design_comparison' || mode === 'design_render_scene') ? 1536 : 1024;
  let height = (mode === 'cinematic_storyboard' || mode === 'setup_comparison' || mode === 'design_comparison' || mode === 'design_render_scene') ? 864 : 1024;
...
async function buildClientCollage(images) {
  if (selectedMode === 'setup_comparison' || selectedMode === 'design_comparison') {
    ...
    if (selectedMode === 'setup_comparison') {
      ...
      drawLabel('平面设计效果图', 0);
      drawCover(designImage, 0, labelH, canvas.width, imageH);
      drawLabel('婚礼现场实景图', halfH);
      drawCover(liveImage, 0, halfH + labelH, canvas.width, imageH);
    }
...
function createCopy(mode) {
  ...
  if (mode === 'design_comparison') {
    return {
      title: '设计图和现场图对比太适合收藏✨',
      body: `设计图和现场图放在一起看，最能看出方案有没有真正落地。舞台比例、花艺位置、灯光氛围和整体色系能不能对上，一眼就能判断。\n\n备婚时很建议收藏这种对比：不要只看效果图好不好看，也要看现场是不是保留了原本的设计重点。能落到实景里的方案，才更值得参考。`,
      tags: config.tags,
    };
  }
...
async function runClientMock(runId) {
  ...
  const generateLog = selectedMode === 'cinematic_storyboard'
    ...
    : selectedMode === 'copy_title'
      ...
      : selectedMode === 'design_comparison'
        ? '[generate] 生成 1 张婚礼设计效果图'
...
function renderResult(result) {
  ...
  if (els.overviewKicker && els.overviewTitle) {
    ...
    const designComparison = mode === 'design_comparison';
    ...
    ? (event) => {
      ...
      const filename = result.mode === 'cinematic_storyboard'
        ...
        : (result.mode === 'setup_comparison'
          ...
          : (result.mode === 'design_comparison' ? 'design-comparison.jpg' : 'wedscene-viral-cover.jpg'));
...
function isComparisonResource(resource = {}) {
  ...
  return resource.mode === 'setup_comparison'
    || resource.mode === 'design_comparison'
    || /comparison|before|after|对比|布置前|布置后|设计图|现场图/i.test(text);
}
...
function resourceVideoAssets(resource) {
  ...
  if (resource.videoUrl) {
    videos.unshift({
      ...
      category: 'videos',
      resource,
      label: resource.modeLabel || '单图运镜视频',
...
function setupVideoPage() {
  ...
  if (els.videoFileInput) els.videoFileInput.multiple = false;
  // 上传区点击/拖拽
...
function renderVideoStyleButtons() {
  const styles = motionConfig.styles?.length ? motionConfig.styles : FALLBACK_MOTION_STYLES;
  if (!styles.find((style) => style.key === videoState.style)) {
    videoState.style = styles[0]?.key || DEFAULT_MOTION_STYLE;
  }
  if (els.videoMotionStyleGrid) {
    els.videoMotionStyleGrid.innerHTML = '';
    styles.forEach((style) => {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = `motion-style-btn ${style.key === videoState.style ? 'active' : ''}`;
      btn.dataset.styleKey = style.key;
      btn.innerHTML = `<strong>${style.label}</strong><span>${style.description || ''}</span>`;
      btn.addEventListener('click', () => {
        videoState.style = style.key;
        els.videoMotionStyleGrid.querySelectorAll('.motion-style-btn').forEach((el) => {
          el.classList.toggle('active', el.dataset.styleKey === videoState.style);
        });
      });
      els.videoMotionStyleGrid.appendChild(btn);
    });
  }
...
function describeVideoUploadAdvice(files, sizes) {
  ...
  const imageSize = validSizes[0];
  if (imageSize && imageSize.width < 720 && imageSize.height < 720) {
    return {
      text: '图片分辨率偏低，成片细节可能不够稳定。建议换成更清晰的原图。',
      warn: true,
    };
  }
  return {
    text: '已选择 1 张图。请选择右侧运镜方式后开始生成。',
    warn: false,
  };
}
...
function renderVideoInputPreviews(files) {
  ...
  if (files.length <= 1) {
    els.videoInputPreviewList.classList.add('hidden');
    return;
  }
  els.videoInputPreviewList.classList.remove('hidden');
  const labels = ['单图参考'];
...
async function handleVideoFiles(files) {
  ...
  els.videoJobStatusText.textContent = '照片已就绪，请选择运镜后开始生成';
  videoUpdateGenerateState();
}
...
function videoUpdateGenerateState() {
  ...
  els.videoGenerateBtn.textContent = videoState.generating
    ? '视频生成中（等待上游）...'
    : `一键生成运镜视频（${motionConfig.pointCost || 60} 灵感值）`;
}
...
function resetVideoWorkflow() {
  ...
  setVideoPosterFallback(els.videoResultVideo, els.videoResultPoster, '');
...
async function startVideoGeneration() {
  ...
  formData.append('motion_style', videoState.style || DEFAULT_MOTION_STYLE);
  if (currentPartnerSlug()) formData.append('partner', currentPartnerSlug());
...
function videoRenderResult(result = {}) {
  ...
  if (els.videoResultVideo) {
    const cacheBustedUrl = cacheBustMediaUrl(videoUrl);
    els.videoResultVideo.src = cacheBustedUrl;
    els.videoResultVideo.preload = 'auto';
    bindVideoPosterFallback(els.videoResultVideo, els.videoResultPoster, posterUrl);
    try { els.videoResultVideo.load(); } catch {}
...
function renderVideoHistory(resources = []) {
  ...
  videos.forEach((resource) => {
    ...
    wrap.className = 'rounded-lg overflow-hidden bg-black aspect-video motion-video-wrap';
    const poster = document.createElement('img');
    poster.className = 'video-result-poster hidden';
    poster.alt = '视频封面';
    const video = document.createElement('video');
    const videoUrl = cacheBustMediaUrl(resource.videoUrl);
    video.src = videoUrl;
    video.controls = true;
    video.preload = 'auto';
    ...
    bindVideoPosterFallback(video, poster, resource.motionPosterUrl || '');
    addVideoFullscreenButton(wrap, video, videoUrl, `wedscene-motion-${resource.motionStyle || 'shot'}.mp4`);
    const meta = document.createElement('div');
    meta.className = 'mt-2 text-xs text-stone-400 font-mono';
    meta.textContent = [
      resource.motionStyleLabel || '单图运镜',
